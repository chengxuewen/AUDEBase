...
import { users, roles, permissions, collections, modules } from "../db/schema.js";
...
interface GraphQLRouteOptions {
  /** Pre-handler auth hook (e.g., Fastify preHandler or onRequest) */
...
export function registerGraphQLRoute(
  ...
  const preHandler = options.authHook ? [options.authHook] : [];
