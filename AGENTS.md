...
AUDEBase — 企业应用开发平台。原为微内核 + 插件热插拔架构（28 packages, 42K 行代码），对标 Odoo、NocoBase。2026-07-21 决策转型：以 NocoBase 为基础平台，将自研创新点（增强 manifest.yaml、Record Rules）作为 NocoBase 插件实现，降低开发成本加速产品化。四层信任分组因 NocoBase 架构限制搁置评估。
...
├── docs/
│   ├── architecture.md # 架构文档（14 章节，Phase 3 NocoBase 转型）
...
│   │   └── specs/
│   │       └── 2026-07-22-phase1a-execution-plan.md  # Phase 3 执行计划
...
│ 架构决策 | `.agents/memorys/decisions.md` | D1-D25 + G1-G5
...
│ Phase 1a 实施计划 | `docs/superpowers/specs/2026-07-22-phase1a-execution-plan.md` | 混合策略：NocoBase 插件 + Canonical Schema 解耦
...
> ⚠️ **归档说明**: 以上 SDD/TDD 文档为 2026-07-17 自建 28 包架构设计参考，已归档至 `docs/archive/`。当前 Phase 3 NocoBase 转型的实施计划见 `docs/superpowers/specs/2026-07-22-phase1a-execution-plan.md`。
### TDD 测试计划格式要求
...
## CODE MAP
_28 包全部实现（插件市场除外），102 测试文件，tsc 全包 clean。Phase 1b 6 包 + Phase 2/4 6 包已接入 CoreApp。_
...
| 管理 UI | ✅ Phase 1a（过渡保留） | `packages/admin-ui/` - 管理后台 React 19 + Ant Design 5（D6-D24） | ✅ admin-ui-sdd.md | ✅ admin-ui-tdd.md |
...
| 模块 | 状态 | 规划路径 | SDD | TDD |
|------|------|----------|-----|-----|
| canonical-schema | 📋 Phase 1a | `packages/canonical-schema/` - 平台无关 JSON Schema 格式（D25.6.8：保留 Zod） | ✅ | ✅ |
| migration-engine (NocoBase) | 📋 Phase 1a | `packages/migration-engine/` - export/import/diff（D25.6.2：往返闸门） | ✅ | ✅ |
| plugin-3d-printer | 📋 Phase 1a | `packages/plugin-3d-printer/` - 3D 打印机 MVP（3 Collections，D25.6.9：砍 Stats/costPerKg/WebSocket） | ✅ | ✅ |
...
|--------|-------------|---------|------|
│ 四层信任分组 | `@audebase/plugin-trust-grouping` | `packages/plugin-framework/` | ⚠️ 搁置（需 fork NocoBase PluginManager 核心） |
| 增强 manifest.yaml | `@audebase/plugin-manifest-enhancer` | `packages/manifest-engine/` | 🔲 待迁移 |
│ Record Rules | `@audebase/plugin-record-rules` | `packages/rbac/` (108 tests) | ✅ Phase 0 Spike 完成 |
| Token Version | 增强 NocoBase JWT 插件 | `packages/auth/` | 🔲 待迁移 |
| 审计增强 | `@audebase/plugin-audit-enhanced` | `packages/audit/` | 🔲 待迁移 |

**NocoBase 原生覆盖**：Schema Engine、工作流引擎、插件市场、多租户。管理 UI 为 Phase 1a 过渡保留（D25.3），Phase 2 逐步迁移至 NocoBase Client。
迁移详情见 `docs/superpowers/specs/2026-07-22-phase1a-execution-plan.md`。
...
- **Phase 3 战略转型** — 2026-07-21 决策基于 NocoBase 重构，详见 `docs/superpowers/specs/2026-07-22-phase1a-execution-plan.md`
