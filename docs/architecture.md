...
> **版本**: 1.0 | **更新日期**: 2026-07-09 | **状态**: Phase 1a-ready — 架构定义完成
...
| **Phase 1a** | 插件框架 + PluginHost + Manifest + plugin-core Bootstrap + 迁移管理 + DB Schema（11 表 DDL）+ JWT + RBAC + 审计日志 + 健康检查 + 管理 UI（插件管理/用户管理 2 页）+ 日志 + CLI（3 命令）+ 速率限制 + 多租户骨架 + i18n 骨架 + API 规范与约定文档 | 🔲 |
| **Phase 1b** | EventBus + Cron + 完整 i18n + 多租户管理 UI + API 版本路由（基础）+ 文件上传/存储 + 通知接口 + CLI 扩展（plugin upgrade）+ D12.1 插件间数据扩展 | 🔲 |
| **Phase 2** | Schema Engine + 进程模式 + Container 隔离 + manifest.exports 契约验证 + 5 状态机 + 完整 Record Rules + WebSocket + 通知渠道实现 + 多租户 Schema 隔离（Phase 1.5） | 🔲 |
