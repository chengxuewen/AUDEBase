...
| 维度 | 组内（同进程） | 组间（跨进程） |
|------|-------------|-------------|
| 方式 | 直接函数调用（~0ms） | JSON-RPC stdin/stdout + Redis Pub/Sub |
| 安全 | 不需要 | Token握手 + nonce防重放 + 帧级认证 |
| 契约 | TypeScript类型 | manifest.exports + Zod schema + ServiceRegistry |

**同步 RPC**: Plugin → Core 路由 → Plugin（2-4ms，含 tenant_id + Record Rules 注入）
**异步事件**: Plugin → Redis Pub/Sub（<1ms fire-and-forget，租户前缀隔离）

**详情**: [插件通信架构](plugin-architecture-analysis.md#插件通信架构)
---
...
**声明（manifest.yaml）** — Phase 1 字段：
name, version, display_name, description, category, license, application, entry, author, dependencies, assets, lifecycle, runtime(mode+partition+crash_policy), security(db_namespace), exports, permissions, models, locale, data

**生命周期** — 借鉴 NocoBase + Odoo：
afterAdd → beforeLoad → load → install → afterEnable → 运行 → afterDisable → pre_uninstall

**迁移三阶段** (NocoBase 模式)：
1. beforeLoad：注册临时表
2. afterSync：Drizzle DDL
3. afterLoad：验证

**依赖解析**: DAG 拓扑排序 (Kahn's algorithm) + semver 校验 + 循环检测

**插件进程隔离**：采用四层信任分组模型（详见 §2.1），50 插件约 8-12 进程 / 0.4-0.7GB。

**详情**: [插件架构分析](plugin-architecture-analysis.md)
...
**Record Rules（记录级权限）**: Odoo domain filter 表达式，Core ORM 自动注入 WHERE 条件
**字段级权限**: manifest.exports 中 visible_to 声明，Core API 响应自动过滤
...
仅 `manifest.security.db_direct: true` 的 Isolated 插件例外（需额外审核）。
...
NocoBase 命名空间模式：插件 locale/{lang}.json → Core 聚合 → context.t() 注入
Phase 1: 预加载所有翻译
### 4.8 通知系统（Phase 2）
...
| **Phase 2** | Schema Engine + 进程模式 + manifest.exports 契约 + 版本管理 + 5 状态机 + Record Rules | 🔲 |
| **Phase 3** | 直连通道 + 流式传输 + OpenTelemetry + 请求优先级 + 插件市场 | 🔲 |
| **Phase 4** | 容器隔离 + Core 高可用 + 工作流引擎 + 业务插件套件 | 🔲 |
