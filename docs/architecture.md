...
**详情**: [插件架构分析](analysis/plugin-architecture-analysis.md#四层信任分组模型)
...
每个业务系统（OA/ERP/MES 等）以插件套件形式存在，通过 `manifest.yaml` 声明。详见 [插件架构分析](analysis/plugin-architecture-analysis.md)。

### 2.3 设计原则（详见 decisions.md）

- **不可变性优先（G1）**：永远返回新对象，不就地修改
- **边界验证（D8）**：所有系统边界使用 Zod Schema 验证
- **插件通信**：组内直接函数调用（~0ms），组间 JSON-RPC over stdin/stdout + Redis Pub/Sub。详见 [插件架构分析](analysis/plugin-architecture-analysis.md#插件通信架构)
...
**模块实现状态**：参见 `.agents/memorys/status.md` 模块状态表。28 包全部实现，102 测试文件，tsc 全包 clean。

> 项目当前状态见 [.agents/memorys/status.md](../.agents/memorys/status.md)。实施计划见 `docs/superpowers/specs/2026-07-22-phase1a-execution-plan.md`。
...
|------|---------|------|
| Phase 0 | 基础设施初始化 | ✅ 已完成 |
| Phase 1a | 插件框架 + Manifest + Bootstrap + RBAC + 审计 + 管理 UI + CLI + 多租户骨架 | ✅ 已完成 |
| Phase 1b | EventBus + Cron + i18n + 文件上传 + 通知接口 + API 版本 + 数据扩展 | ✅ 已完成 |
| Phase 2 | Schema Engine + WebSocket + Record Rules/D10 | ✅ 已完成 |
| Phase 4 | 工作流引擎（Saga + 幂等性） | ✅ 已完成 |

### NocoBase 迁移阶段（当前）

> ⚠️ **命名说明**: NocoBase 迁移使用 Phase 1/2/3（来源 D25.4），与上方自建架构 Phase 1a/1b/2/3/4 为独立编号。Phase 1a 执行计划对应 NocoBase 迁移 Phase 1 的首次迭代。

| 阶段 | 状态 | 范围 | 目标 |
|------|------|------|------|
| Phase 1 — 架构对齐 | 📋 计划定稿 | 插件生命周期、manifest 规范、PluginManager API | Phase 0 Spike 完成（2026-07-21），Phase 1a 执行计划已审批（2026-07-22）。详见 `docs/superpowers/specs/2026-07-22-phase1a-execution-plan.md` |
...
### Canonical Schema 数据可移植层

Phase 1a 引入 **Canonical Schema** — 平台无关的 JSON 数据格式，用于 NocoBase 数据导出/导入/对比。

**目的**：解耦业务数据与 NocoBase 内部 Schema，确保数据可在不同后端间迁移。

**三个核心能力：**
- **Export**：NocoBase DB → `CanonicalSnapshot`（JSON，含 Collection + Record + FK 关系）
- **Import**：`CanonicalSnapshot` → NocoBase DB（multi-pass FK 处理 + PG 事务）
- **Diff**：两个 `CanonicalSnapshot` 逐记录对比，返回结构化 `DiffReport`

**包**：`@audebase/canonical-schema`（纯类型 + Zod validators）+ `@audebase/migration-engine`（export/import/diff 实现）。`@audebase/plugin-3d-printer` 为 NocoBase 业务插件（3 Collections + 3 SchemaComponent 页面 + Agent HTTP polling）。
**关键决策**（D25.6）：PostgreSQL 16 强制（非 SQLite）、Canonical Schema 往返闸门（export→import→export→diff 必须 return 0 diffs）、multi-pass FK 导入（SET CONSTRAINTS ALL DEFERRED）、Agent HTTP polling（5s interval 替代 WebSocket）、砍 StatsPage/costPerKg/WebSocket 实时推送。
详见 `docs/superpowers/specs/2026-07-22-phase1a-execution-plan.md` §3.1–§3.2。

## 九、决策记录索引

以下架构决策已记录于 `.agents/memorys/decisions.md`（共 50+ 条）：
...
| D1.1 | 四层信任分组模型 | ⚠️ 搁置（D25） | SYSTEM/Domain/Isolated/Container（需 fork NocoBase 核心） |
...
### NocoBase 战略转型决策（Phase 3）

| ID | 决策 | 状态 | 说明 |
|----|------|------|------|
| D25 | 基于 NocoBase 重构 | 📋 Phase 1a 执行中 | 以 NocoBase v2.0 为核心，自研创新作为 NocoBase 插件 |
| D25.1 | NocoBase 覆盖范围 | 已决策 | 10 大能力直接采用 NocoBase 原生 |
| D25.2 | AUDEBase 上层创新 | 已决策 | 7 项差异化创新：Record Rules / manifest 增强 / Saga / ProTable 路径等 |
| D25.3 | 现有代码保留与替换 | 已决策 | 15 包保留转化 → 11 包替换 → admin-ui 过渡保留 |
| D25.4 | 迁移阶段 | 📋 Phase 1a | Phase 1a 插件 MVP → Phase 2 数据模型 → Phase 3 生态兼容 |
| D25.5 | 风险与缓解 | 已决策 | 5 项风险 + 缓解（版本锁定、插件迁移、ORM 成本等） |
| D25.6 | Phase 1a 混合策略 | 📋 计划定稿 | 10 子决策：PG16 强制 / round-trip gate / Agent HTTP polling / scope cuts / 测试策略等 |
...
- 插件架构深度分析: [plugin-architecture-analysis.md](analysis/plugin-architecture-analysis.md)（~850 行，4 轮团队审核，8 项目对比）
...
- **竞品调研**: [competitive-landscape.md](analysis/competitive-landscape.md) — 39+ 产品对标分析，五大类别
...
- **模块设计文档**: `docs/modules/` — plugin-framework.md、plugin-communication.md、multi-tenant.md、file-storage.md、database-schema.md、api-specification.md、api-conventions.md、dev-workflow.md、frontend-spec.md、migration-engine-sdd.md、plugin-framework-sdd.md、test-seed-strategy.md、e2e-test-flows.md、redis-mock-guide.md
- **插件架构分析**: [plugin-architecture-analysis.md](analysis/plugin-architecture-analysis.md) — ~850 行，四层模型设计，8 项目对比
