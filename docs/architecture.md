...
> **版本**: 4.0 | **更新日期**: 2026-07-23 | **状态**: D26 — Refine+ProLayout 混合架构 + 自建 15 包底座

> ⚠️ **D26 架构变更声明** (2026-07-23)
> AUDEBase 放弃 Fork NocoBase（D25），转向 Refine 数据层 + ProLayout 外壳 + 自建 Schema→UI 映射器。后端裁剪为 15 包底座（Fastify + Drizzle + PostgreSQL）。D25 废弃。设计见 `docs/superpowers/specs/2026-07-23-refine-hybrid-architecture-design.md`。
...
**NocoBase 的 SchemaComponent/ACL 设计思想是 AUDEBase 映射器与权限模型的设计参考**（运行时不依赖 NocoBase）。
...
AUDEBase 自建架构由 28 个独立 npm 包组成（D26 裁剪为 15 包底座），按职责分为五层：**内核** → **插件基础设施** → **横切关注点** → **数据引擎** → **扩展模块**。
...
### 6.4 Schema 驱动 UI（D26 Phase 1a）

D26 Phase 1a 自研 Schema→UI 映射器（Refine + ProTable/ProForm 集成）。不引入 Formily。
...
**模块实现状态**：参见 `.agents/memorys/status.md` 模块状态表。28 包全部实现（自建架构），D26 裁剪为 15 包底座。

> 项目当前状态见 [.agents/memorys/status.md](../.agents/memorys/status.md)。实施计划见 `docs/superpowers/plans/2026-07-23-refine-hybrid-implementation-plan.md`。
...
| Phase 3 | NocoBase 迁移与生态兼容（D25） | ⚠️ 已废弃（D26） |

### D26 混合架构阶段（当前）

D26（2026-07-23）废弃 D25，采用 Refine + ProLayout + 自建 15 包底座。

**保留的 D25 决策**：Agent HTTP polling (D25.6.1)、Canonical Schema 往返闸门 (D25.6.2，Drizzle 实现)、PostgreSQL 16 强制 (D25.6.7)、削减范围 (D25.6.9)。

| 模块 | 状态 | 规划路径 |
|------|------|----------|
| Refine 集成 | 🔲 Phase 1a | `packages/admin-ui/` — dataProvider + authProvider + Schema→UI Mapper |
| 3D 打印机 MES | 🔲 Phase 1a | `packages/admin-ui/src/pages/workspace/` — 打印任务/设备管理/材料库存/仪表盘 |
| Canonical Schema | 🔲 Phase 2 | 改为 Drizzle 实现 |
...
| D1.1 | 四层信任分组模型 | ⚠️ 搁置（D25→D26 保留搁置） | SYSTEM/Domain/Isolated/Container（需 fork NocoBase 核心） |
...
| ID | 决策 | 状态 | 说明 |
|----|------|------|------|
| D25 | 基于 NocoBase 重构 | ⚠️ 已废弃（D26） | D25 废弃，D26 Refine+ProLayout+15包底座替代 |
| D26 | Refine+ProLayout 混合架构 | ✅ 设计定稿 | 前端 Refine 数据层 + ProLayout 外壳 + Schema→UI 映射器，后端 15 包底座（Fastify+Drizzle+PostgreSQL） |
