...
> **创建日期**: 2026-07-13 | **最后更新**: 2026-07-19
...
| Phase        | 代号             | 目标                                                                                  |      预计工时      |          状态           |
| ------------ | ---------------- | ------------------------------------------------------------------------------------- | :----------------: | :---------------------: |
| **Week 0**   | CI/CD & 基础设施 | Turborepo + Vitest + Playwright + GitHub Actions + Docker Compose + antd 5 兼容性验证 |       ~4.5天       |        ✅ 已完成        |
| **Phase 1a** | 内核 MVP         | 12 核心模块 + Admin UI，384 tests，插件框架 → 用户管理 → 日志 端到端                  | 4-6 周（4 人并行） |        ✅ 已完成        |
| **Phase 1b** | 平台能力扩展     | EventBus + Cron + 插件通信 + API 版本化 + 通知接口 + 文件存储隔离                     |       3-4 周       |        ✅ 已完成        |
| **Phase 2**  | Schema Engine    | 动态模型定义 + Schema→DB + Schema→UI + 进程隔离 + WebSocket                           |        待定        |        ✅ 已完成        |
| **Phase 3**  | 生态与合规       | OpenTelemetry + 插件市场 + Database-per-tenant + 混合模式                             |        待定        |           🔲            |
| **Phase 4**  | 高可用与业务     | Core HA + 工作流引擎（workflow-core/engine/tasks）+ 业务插件套件                      |        待定        | ✅ 已完成（工作流引擎） |
