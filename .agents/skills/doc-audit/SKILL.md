---
name: doc-audit
description: "AUDEBase 项目文档与架构审计。并行检查架构文档、设计文档、决策记录之间的自洽性、完整性、缺口和优化机会。以交互式问答与用户逐项确认每项发现，列出详情、方案与优劣、来源、影响、推荐。支持团队模式(大型审计)和单线程模式(轻量检查)。"
---

# 文档架构审计 (Document & Architecture Audit)

对 AUDEBase 项目文档体系进行全面审计。
...
[3] 文档一致性 — arch ↔ module docs ↔ memory files 交叉检查
[4] 参考验证 — reference/ 竞品调研 ↔ AUDEBase 设计
[5] 缺口优化 — 运维/安全/可靠性扫描
[6] 阶段审计 — SDD / TDD / 实施计划
```
...
核查 `decisions.md` D1-D24, G1-G5 在 `docs/architecture.md`、`docs/modules/`、`conventions.md`、`status.md` 中的落实。
...
- **决策新鲜度检查**：
  - 该决策是否被后续决策明确替代？
  - 决策中的技术栈版本是否过期？（Drizzle 0.45.x vs 1.0、React 19 vs 20）
  - 决策理由是否仍然成立？（如 "Phase 1 inline 模式" → Phase 2+ 是否需要更新？）

### 2. 文档一致性 (Consistency-Checker)
核查 `docs/architecture.md` ↔ `docs/modules/*.md` ↔ `.agents/memorys/` ↔ `AGENTS.md` 交叉一致性。

**核心问题**：
- 同一概念描述是否一致？（技术栈、Phase 定义、模块状态）
- 子文档是否与主文档重复？是否需要折叠为引用？
- Phase 术语是否歧义？（Phase 1a vs Phase 1b vs Phase 2 状态标记）
- 命名空间/包名一致性（`@audebase/` scope）
- 模块状态表在 status.md、AGENTS.md、README.md 中是否一致

### 3. 参考验证 (Reference-Crosschecker)
核查 `docs/reference/` 中 15 份竞品产品画像的「对 AUDEBase 参考价值」→ AUDEBase 设计吸收情况。

**核心问题**：
- 多篇竞品一致推荐的功能/模式是否已吸收？
- 关键发现是否与当前设计矛盾？
- NocoBase/Odoo/Strapi/Directus 等已验证模式是否映射到 AUDEBase 架构？
...
- 安全架构：JWT 密钥管理（D8.1）、CVE 防范措施、审计日志完整性
- 错误模型：插件加载失败、迁移失败、租户隔离失效、BullMQ 队列溢出
- 部署基线：最低 Node.js 版本、PostgreSQL 版本、Redis 版本
- 资源限制：每插件内存预算、连接池配置
- 升级策略：插件热更新、manifest 版本升级、数据库迁移回滚

### 5. 阶段审计 (Phase Audit)

#### 5a. SDD 文档审计
核查 SDD 文档（`docs/modules/*-sdd.md`）是否覆盖决策记录中的所有 API 契约。

**核心问题**：
- SDD 是否覆盖了所有已决策接口（PluginHost、PermissionEngine、I18nEngine 等）？
- 边界条件是否枚举完整？
- 验收标准是否可测量、可验证？
- Mock 约束是否满足 5 项要求（async Promise、JSON 序列化、30s 超时、延迟注入）？

#### 5b. TDD 测试审计
核查 TDD 测试计划（`docs/modules/*-tdd.md`）是否覆盖 SDD 的全部验收标准。
...
#### 5c. 实施计划审计
核查实施计划是否与 `docs/phase-planning.md` 对齐。
...
| 🟠 HIGH | 陈旧引用 / Phase 歧义 / 重复文档 / 模块状态不一致 | ⚠️ |
