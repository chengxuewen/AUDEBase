...
- **状态**: 已决策，Phase 1 实现 inline 模式（Phase 1a），Phase 2 process 和 container 隔离
...
- **状态**: 已决策，Phase 1a 实现（发现 #1：从 1b 提前到 1a，作为 plugin-core Bootstrap 后第一个可执行迁移的框架）
...
- **状态**: 已决策，Phase 1b 实现同进程 EventBus
...
- **状态**: 已决策，Phase 1b 接口定义，Phase 2 渠道实现
...
- **状态**: 已决策，Phase 1b 实现基础 extends 声明解析 + 字段合并
### D13: Saga 跨插件事务
