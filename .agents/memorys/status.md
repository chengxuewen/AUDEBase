...
**更新日期**: 2026-07-23
**当前阶段**: D26 实施完成 — Refine+ProLayout 混合架构，10 Task 全部完成。28 包全部实现，D25 NocoBase 方案已废弃。
...
AUDEBase 决定采用 NocoBase 作为核心架构参考，在保留现有微内核 + 插件热插拔架构的基础上，逐步对齐 NocoBase 的插件模型、数据模型和生态体系。

### 迁移阶段

| 阶段 | 状态 | 范围 | 目标 |
|------|------|------|------|
| Phase 1a — NocoBase 插件 MVP | 📋 计划定稿 | canonical-schema + migration-engine + plugin-3d-printer | Phase 0 Spike 完成（2026-07-21），execution-plan 已审批（2026-07-22）
...
| ⚠️ | 四层信任分组 | 搁置 — 需 fork NocoBase PluginManager 核心，无进程隔离架构可承载 |
## 模块状态
...
| 插件通信 | ✅ Phase 1b | 组内直调 + 组间 JSON-RPC + Redis Pub/Sub（D1.3） - 3 测试文件 |
...
| WebSocket | ✅ Phase 2 | Collection 变更事件订阅（D1.11） - 2 测试文件 |
| Schema Engine | ✅ Phase 2 | 动态模型定义 + Schema→DB DDL + Schema→UI 映射（D3/D7） - 5 测试文件 |
...
| 工作流引擎 | ✅ Phase 4 | Saga 补偿模式 + 工作流定义/执行/任务（D13） - 4 测试文件 |
...
| 模块 | 状态 | 描述 |
|------|------|------|
| canonical-schema | 📋 Phase 1a | 平台无关 JSON Schema 格式 — SDD+TDD 完成，编码 🔲
| migration-engine (NocoBase) | 📋 Phase 1a | Canonical Schema export/import/diff + topo-sort — SDD+TDD 完成，编码 🔲
| plugin-3d-printer | 📋 Phase 1a | 3D 打印机 MVP 业务插件 + Agent HTTP polling — SDD+TDD 完成，编码 🔲
| plugin-record-rules | ✅ Phase 0 Spike | Poland-notation 解析器 + Sequelize WhereOptions 生成器 + ACL 中间件（575 行） |
...
| 日志/调试 | ✅ logging-infra-sdd.md | ✅ logging-infra-tdd.md | ✅ 34 测试通过 | ✅ 完成 |
 Core 内核骨架 | ✅ core-sdd.md | ✅ TDD 完成 | ✅ 34 测试通过 | ✅ 完成 |
 JWT 认证 | ✅ auth-sdd.md | ✅ TDD 完成 | ✅ 34 测试通过 | ✅ 完成 |
 CLI 工具 | ✅ cli-sdd.md | ✅ TDD 完成 | ✅ 16 测试通过 | ✅ 完成 |
 速率限制 | ✅ rate-limit-sdd.md | ✅ TDD 完成 | ✅ 19 测试通过 | ✅ 完成 |

### Phase 1a 新包 SDD/TDD 文档（NocoBase 迁移）

| 模块 | SDD 文档 | TDD 测试计划 | 编码状态 | AI 工作流状态 |
|------|---------|-------------|---------|-------------|
| canonical-schema | ✅ canonical-schema-sdd.md | ✅ canonical-schema-tdd.md | 🔲 待编码 (1.5d) | ✅ SDD+TDD 完成 |
| migration-engine | ✅ migration-engine-sdd.md | ✅ migration-engine-tdd.md | 🔲 待编码 (8d) | ✅ SDD+TDD 完成 |
| plugin-3d-printer | ✅ plugin-3d-printer-sdd.md | ✅ plugin-3d-printer-tdd.md | 🔲 待编码 (10d) | ✅ SDD+TDD 完成 |
...
## Phase 2 SDD/TDD 文档状态

| 模块 | SDD 文档 | TDD 测试计划 | 编码状态 | AI 工作流状态 |
|------|---------|-------------|---------|-------------|
| Schema Engine | ✅ schema-engine-sdd.md | ✅ schema-engine-tdd.md | ✅ 5 测试文件 | ✅ 完成 |
| WebSocket | ✅ websocket-sdd.md | ✅ websocket-tdd.md | ✅ 2 测试文件 | ✅ 完成 |

**注**: SDD 文档已归档至 docs/archive/；活跃参考文档位于 docs/modules/。TDD 测试计划由 AI 代理在编码前根据 SDD 接口定义生成。详见 AGENTS.md。
...
**注**: Phase 3 NocoBase 迁移（生态兼容），OpenTelemetry + 插件市场 + Database-per-tenant 已改为 NocoBase 原生覆盖（D25.1）。

## 已知缺失

- ~~无 build scripts~~ ✅ 已修复 - package.json 含 pnpm dev/build/test scripts
- E2E 测试基础设施已就绪（Playwright config + auth fixtures + 5 测试文件 + CI workflow），浏览器下载未完成需 `headless: false`
- ~~无测试基础设施~~ ✅ 已修复 - vitest 3.0.5 + @testing-library/react 16.2.0 + @playwright/test 1.50.1 已安装，752 tests
- ~~shared-types 包未创建~~ ✅ 已修复 - shared-types 包 Phase 1a 完成，53 测试通过
...
- GO-026 (MEDIUM, PARTIAL): CI 存在且已补 E2E workflow — 仍缺 npm audit + 迁移预检 + 覆盖率闸门
...
- PA-004 (HIGH, FIXED): packages/ 目录结构三方不一致 - master-plan 说独立目录, execution-guide 说 packages/core/, AGENTS.md CODE MAP 独立目录 - 待 Phase 1a Week 0 确认
...
| 组件 | 状态 | 说明 |
|------|:----:|------|
| Playwright config | ✅ | setup project + storageState + headless:false |
| Auth fixtures | ✅ | global-setup.ts + auth.setup.ts + fixtures.ts |
| Login E2E | ✅ | 5 场景（成功/失败/空字段/锁定/登出） |
| Dashboard E2E | ✅ | 4 场景（渲染/菜单/导航/租户信息） |
| Users E2E | ✅ | 7 场景（CRUD + 分页 + 搜索） |
| Roles E2E | ✅ | 5 场景（CRUD + 权限变更） |
| Plugins E2E | ✅ | 6 场景（列表/启用/禁用/安装/卸载/详情） |
| CI workflow | ✅ | .github/workflows/e2e.yml（PostgreSQL + Valkey） |
| data-testid | ✅ | Login/Dashboard/Users/Roles/Plugins 5 页面 |
| 浏览器 | ⚠️ | chromium_headless_shell 下载未完成 |
| local-openspace  |  ✅  | OpenSpace MCP（从 AUDESYS 移植并修复连接问题）       |
...
- 2026-07-09: 文档 docs/analysis/plugin-architecture-analysis.md 完成
...
- 2026-07-17: core 模块完成 (已重命名) — Fastify + Drizzle + JWT auth + CRUD API + CLI + 启动管线
...
- 2026-07-13: docs/plans/ 创建 5 份 Phase 1a 执行计划文档（~160KB）（docs/plans/ 已删除，Phase 1a 计划已完成）
...
- 2026-07-19: Phase 2 完成 — schema-engine（动态模型定义 + Schema→DB DDL + Schema→UI 映射）+ WebSocket（Collection 变更事件订阅）
- 2026-07-19: Phase 4 工作流引擎完成 — workflow-core/engine/tasks（Saga 补偿模式 + 幂等性）
- 2026-07-19: merge plan A — 合并 origin/main（bkg 分支）和 feature/dev 分支，28 包统一，kernel→core 重命名
- 2026-07-19: 功能 port — GraphQL API 端点（POST /api/graphql）、Record Rules D10 完整实现（parseDomainFilter + generateWhereClause + 108 tests）、plugin CLI 命令、notification providers（Email + InApp）
- 2026-07-20: 文档审计完成 — 5 人团队（consistency-checker/decision-validator/phase-auditor/reference-crosschecker/gap-optimizer），73+ 发现去重
- 2026-07-20: 6 CRITICAL 修复 — 包数 22→28、包名 kernel→core、Phase 声明更新、CVE-2026-52888 录入、模块表追加 6 包、Valkey 8 版本修正
- 2026-07-20: 3 决策冲突解决 — Phase 3 定义对齐 phase-planning.md（Docker Compose→OpenTelemetry，✅→🔲）、Phase 4 补记业务插件延后、D10 标记已实现
- 2026-07-20: cleanup — migration-engine 死目录删除、Dockerfile/docker-bootstrap.sh kernel→core 引用更新、3 份文档同步（AGENTS.md/status.md/architecture.md）
- 2026-07-21: ACP 插件移除 — 删除 .opencode/acp.jsonc，清理 7 处引用
- 2026-07-21: doc-audit — 31 项发现全部修复，10 文件更新
- 2026-07-21: docs/ 重组 — plans/ → roadmap/，analysis/ 目录，删除 3 个废弃模块桩
- 2026-07-21: SDD+TDD 生成 — schema-engine + websocket 模块文档完成
- 2026-07-21: E2E 基础设施 — Playwright config + auth fixtures + 5 测试文件（~23 场景）+ CI workflow + data-testid 属性
- 2026-07-21: Bug 修复 — App.tsx token 过期检查、AdminLayout.tsx 菜单 null-guard、vite proxy 3001→5110、E2E selectors data-testid 化
- 2026-07-21: 认证修复 — 新增 GET /api/auth/me 端点、App.tsx 3 状态阻塞式认证（loading→验证→渲染）、verifyAccessToken 按 sub 过滤用户（修复预存 token_version 不匹配 bug）、createAuthDbAdapter 支持 findFirst args 透传
- 2026-07-21: E2E 测试增强 — login.e2e.ts 新增 stale token 恢复 + 有效 token 加载验证场景
- 2026-07-21: 测试验证 — vitest 120 passed, curl 验证 /api/auth/me 正常, tsc admin-ui 编译通过
- 2026-07-21: NocoBase 战略转型决策 — 确定以 NocoBase 为核心架构参考，制定三阶段迁移计划（架构对齐 → 数据模型对齐 → 生态兼容）
- 2026-07-21: Hyperplan 对抗式审查 — 4 角色（risk-hunter/scope-auditor/tech-challenger/minimalist）审查迁移方案，发现 3 个架构不可行项（trust-grouping 需 fork 核心、Drizzle→Sequelize ORM 鸿沟需重写、范围低估 5x），制定校准执行计划
- 2026-07-21: Phase 0 Spike 完成 — NocoBase v2.1.29 分析完成，确认 acl.use() 可注入 Record Rules；@audebase/plugin-record-rules 实现（575 行，含 Poland-notation 解析器 + Sequelize WhereOptions 生成器 + ACL 中间件集成）
- 2026-07-22: 平台接口设计 — 7 核心接口 (IPlatform/IDataEngine/IACL/IPluginSystem/IWorkflow/IAuth/ITenant) + Canonical Schema + IMigrationEngine，混合策略（NocoBase 插件 + Canonical Schema 解耦）
- 2026-07-22: 双 Oracle + 4 角色团队审查 — 发现 4 CRITICAL（事务/倒灌/分页/OOM）→ pivot 到混合策略；Phase 1a 执行计划 29d 校准 → 21d 单人 / 3.5w 双人
- 2026-07-22: Phase 1a 执行计划定稿 — canonical-schema（1d）+ migration-engine（8d）+ plugin-record-rules（已完成）+ plugin-3d-printer（10d）+ Agent HTTP polling（1.5d）
- 2026-07-22: docs/ 清理归档 — 删除 5 过时文件，归档 51 SDD/TDD → docs/archive/，4 文档更新路径引用
- 2026-07-22: 文档全量审计 — 4 角色 teams 并行审计（36 项发现），28 项修复归档（8 文件），D25 决策传播、Phase 路线图重写、新 NocoBase 包注册、信任分组搁置标注
- 2026-07-22: D25.6 录入 decisions.md + plan 补充里程碑验收检查点 + 测试策略 + doc-audit 修复 6 项发现
- 2026-07-22: SDD+TDD 生成 — canonical-schema (SDD 145 行 + TDD 142 行，5 测试) + migration-engine (SDD 305 行 + TDD 792 行，35 测试) + plugin-3d-printer (SDD 325 行 + TDD 546 行，24 测试) — 全部 4 包 SDD+TDD 就绪
- 2026-07-22: 3 包状态更新 — Phase 3 NocoBase 迁移包 canonical-schema / migration-engine / plugin-3d-printer 🔲→📋（SDD+TDD 完成，等待编码）
- 2026-07-22: 执行计划 doc-audit — 20 项发现全部修复（6C+7H+4M+3L）：健康检查/日志检查 M1 补项 + 安全审计+DB 备份 M2 补项 + E2E 冒烟去重 + 备份策略风险行 + CVE D25 映射 + plugin-architecture-analysis 搁置标注 + 里程碑数量同步 + 4 文档 14 处编辑
- 2026-07-23: D26 实施完成 — 10 Task 全部完成：管理 UI 迁移至 Refine + ProLayout + Schema→UI Mapper，后端裁剪至 15 包底座，前端重写为 RefineProvider + collection-driven pages，旧页面清理归档
- 2026-07-23: D26 Phase 9 — 旧管理 UI 页面测试归档 + 死代码删除（DashboardPage/UserManagementPage/RoleManagementPage/PluginManagementPage/SchemaPage）+ 文档状态更新
